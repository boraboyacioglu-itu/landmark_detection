...
class LandmarkExtractor(nn.Module):
    def __init__(self, n_landmark = 478, output_dim = 512):
        super(LandmarkExtractor, self).__init__()

        self.fc1 = nn.Linear(n_landmark * 3, 1024)
...
class ModifiedGDC_Landmarks(nn.Module):
    def __init__(self, image_size, in_chs, num_classes, dropout, emb=512, n_landmark=1434): # 478*3 = 1434: # dropout implementation is in the original code but not in the paper
        ...
        # Include the dimension of the landmark features in the linear layer
        self.linear = nn.Linear(emb + n_landmark*3, num_classes) if num_classes else nn.Identity()
    ...
    def forward(self, x, landmarks):
        ...
        # Concatenate image features with landmark features
        x = torch.cat((x, landmarks), dim=1)
        x = self.linear(x)
        return x
    
class ModifiedGDC(nn.Module):
    def __init__(self, image_size, in_chs, dropout, emb=512): # dropout implementation is in the original code but not in the paper
        super(ModifiedGDC, self).__init__()
...
class GhostFaceNetsV2(nn.Module):
    def __init__(self, cfgs=None, image_size=256, num_classes=0, width=1.0, channels=3, dropout=0.2, combined = 'combined', n_landmark=478, block=GhostBottleneckV2,
                 add_pointwise_conv=False, bn_momentum=0.9, bn_epsilon=1e-5, emb= 512, init_kaiming=True, args=None):
        ...
        if combined == 'Image':
            self.combined = 'Image'
            self.image_extractor = ModifiedGDC(image_size, output_channel, dropout)
            self.linear = nn.Linear(emb, num_classes) if num_classes else nn.Identity()
        elif combined == 'Landmark':
            self.combined = 'Landmark'
            self.landmark_extractor = LandmarkExtractor(n_landmark=n_landmark, output_dim=emb)
            self.linear = nn.Linear(emb, num_classes) if num_classes else nn.Identity()
        elif combined == 'Combined':
            self.combined = 'Combined'
            self.image_extractor = ModifiedGDC(image_size, output_channel, dropout)
            self.landmark_extractor = LandmarkExtractor(n_landmark=n_landmark, output_dim=emb)
            self.linear = nn.Linear(emb + emb, num_classes) if num_classes else nn.Identity()
        # Initialize weights
    ...
    def forward(self, x=None, landmarks=None):
        if self.combined == 'Landmark':
            landmarks = self.landmark_extractor(landmarks)
            landmarks = self.linear(x)
            x = landmarks
        else:
            x = self.conv_stem(x)
            x = self.bn1(x)
            ...
            if self.combined == 'Image':
                x = self.linear(x)
            elif self.combined == 'Combined':
                landmarks = self.landmark_extractor(landmarks)
                x = torch.cat((x, landmarks), dim=1)
                x = self.linear(x)
        return x